...
The task of root cause diagnosis is to learn a classification model $f_\theta$ on $\mathcal{D}_{train}$ that can assign the root cause $y_t$ for any newly observed time series $\tilde{w}_t$. To achieve this, we propose a two-stage architecture comprising i) an \textbf{anomaly detection stage} and ii) a \textbf{root cause classification stage}.
...
\begin{figure*}[!ht]
    ...
    \caption{Cloud VR RCD framework}
    \label{fig:chap_6:rcd_framework}
\end{figure*}
